...
async function main(): Promise<void> {
  ...
  try {
    ...
    const encoderUrl = webgpu
      ? "/models/transformer_encoder_model_fp16.onnx"
      : "/models/transformer_encoder_model_fp32.onnx";
    log(`WebGPU ${webgpu ? "available" : "unavailable"} -> ${webgpu ? "fp16" : "fp32"} encoder`);
    ...
    const encoderName = webgpu ? "fp16 (WebGPU)" : "fp32 (WASM)";
    ...
    done({ webgpu: workerWebGpu, encoder: encoderName, symbolCount: result.symbols.length, tokenDistance, midiDistance, midiBytes: size, midiFormatOk: formatOk, layoutSource: result.layoutSource, pass });
  } catch (e) {
    done({ webgpu: false, encoder: "?", symbolCount: 0, tokenDistance: -1, midiDistance: -1, midiBytes: 0, midiFormatOk: false, layoutSource: "?", pass: false, error: e instanceof Error ? e.message : String(e) });
  }
}

...
export async function loadModels(
  ...
  const gpuFirst = gpu ? ["webgpu", "wasm"] : ["wasm"];
  const [encoder, decoder, segnet] = await Promise.all([
    createSession(urls.encoderUrl, gpuFirst, onProgress),
    createSession(urls.decoderUrl, ["wasm"], onProgress),
    urls.segnetUrl ? createSession(urls.segnetUrl, gpuFirst, onProgress) : Promise.resolve(undefined),
  ]);
  return { encoder, decoder, segnet };
}

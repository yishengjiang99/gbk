...
export async function encodeStaff(session: SessionLike, input: Float32Array): Promise<EncoderContext> {
  const outputs = await session.run({
    [ENCODER_INPUT_NAME]: { dims: [1, 1, ENCODER_HEIGHT, ENCODER_WIDTH], float32Data: input },
  });

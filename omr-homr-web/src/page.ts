...
import { SegNetMasks, SEGNET_WINDOW, masksFromLabels, runSegNet } from "./geometry/segnet.ts";
import {
  NoteheadWithStem,
  addNotesToStaffs,
  autocrop,
...
export async function extractPagePredictions(
  ...
  // homr's inference defaults step to windowSize / 2 (160): overlapping
  // tiles, averaged and truncated on stitch.
  const labels = await runSegNet(segnet, preprocessed, {
    stepSize: SEGNET_WINDOW / 2,
    inputFloat16,
...
// ---------------------------------------------------------------------------
// Note layout (attention boxes mapped to page coordinates)
// ---------------------------------------------------------------------------

/** One sounding note with an optional page-space box. */
export interface PageNoteLayoutEntry {
  pitch: number;
  startSec: number;
  endSec: number;
...
export const ATTENTION_BOX_SIZE = 24;

const SECONDS_PER_TICK = 60 / SCAN_TEMPO_BPM / TICKS_PER_QUARTER;

/**
 * Builds one layout entry per sounding note event. Each voice's symbols are
 * split back into per-staff groups (splitVoiceByStaff) and re-decoded with a
 * running tick cursor so event ticks match the flattened MIDI timeline; the
 * decoder attention point is then mapped through the staff's crop->page
 * chain. Boxes are coarse (documented); a missing attention point or crop
 * mapping yields a null box, which callers report as layoutSource
 * "midi-fallback".
 */
export function buildPageNoteLayout(
  voices: ParsedVoice[],
): { layout: PageNoteLayoutEntry[]; attentionBoxes: number; warnings: string[] } {
  const layout: PageNoteLayoutEntry[] = [];
  ...
  voices.forEach((voice, voiceIdx) => {
    ...
    groups.forEach((group, groupIdx) => {
      ...
      for (const ev of events) {
        ...
        let box: PageNoteLayoutEntry["box"] = null;
        if (ev.attention && toPage) {
          const [ax, ay] = ev.attention;
          const half = ATTENTION_BOX_SIZE / 2;
          const [x0, y0] = toPage(ax - half, ay - half);
          const [x1, y1] = toPage(ax + half, ay + half);
          box = {
            x: Math.min(x0, x1),
            y: Math.min(y0, y1),
            w: Math.abs(x1 - x0),
        ...
        layout.push({
          pitch: ev.midi,
          startSec: tick * SECONDS_PER_TICK,
          endSec: (tick + ev.durationTicks) * SECONDS_PER_TICK,
          box,
        });
      }
      cursor += endTick;
  ...
  layout.sort((a, b) => a.startSec - b.startSec || a.pitch - b.pitch);
  return { layout, attentionBoxes, warnings };
}

...
/**
 * Mean absolute convolution response. cv2.filter2D with ddepth=-1 keeps the
 * uint8 depth, so negative responses saturate to 0 and responses above 255
 * saturate to 255 before the abs/mean — replicated here.
 */
export function estimateNoise(image: GrayImage): number {
  ...
  for (let i = 0; i < d.length; i++) {
    const v = d[i] < 0 ? 0 : d[i] > 255 ? 255 : d[i];
    sum += v;
  }
  return sum / d.length;
}
...
/**
 * Zero out whole prediction masks whose noise estimate exceeds the
 * threshold (in-place, like homr's filter_predictions).
 */
export function filterPredictions(masks: SegNetMasks, noiseThreshold: number): void {
  const scaled = (m: GrayImage): GrayImage => {
    const out = GrayImage.zeros(m.width, m.height);
    for (let i = 0; i < m.data.length; i++) out.data[i] = m.data[i] ? 255 : 0;
    return out;
  };
  const zero = (m: GrayImage): void => {
    m.data.fill(0);
  };
  if (estimateNoise(scaled(masks.staff)) > noiseThreshold) zero(masks.staff);
  if (estimateNoise(scaled(masks.symbols)) > noiseThreshold) zero(masks.symbols);
  if (estimateNoise(scaled(masks.stemsRest)) > noiseThreshold) zero(masks.stemsRest);
  if (estimateNoise(scaled(masks.notehead)) > noiseThreshold) zero(masks.notehead);
  if (estimateNoise(scaled(masks.clefsKeys)) > noiseThreshold) zero(masks.clefsKeys);
}

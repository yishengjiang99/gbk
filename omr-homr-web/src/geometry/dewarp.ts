 ...
 * point grids. Direct port of calculate_span_and_optimal_points.
 */
export function calculateSpanAndOptimalPoints(
  ...
  const lines: Pt[][] = [[], [], [], [], [], []];
  for (let x = 0; x < imageWidth; x += 80) {
    const point = staff.getAt(x);
    if (point === null) continue;
    for (let lineIndex = 0; lineIndex < 6; lineIndex++) {
      const y = point.y[lineIndex] - point.y[2];
      lines[lineIndex].push([x, y]);
    }
  }
  if (lines[3].length === 0) throw new Error("No staff points for dewarping");
  const spanPoints: Pt[][] = [];
  const optimalPoints: Pt[][] = [];
  for (const line of lines) {
    if (line.length === 0) throw new Error("Empty dewarp line");
    const averageY = line.reduce((s, p) => s + p[1], 0) / line.length;
    const spanLine: Pt[] = line.map(([x, y]) => [x, y + averageY] as Pt);
    spanLine.unshift([0, spanLine[0][1]]);
    spanLine.push([imageWidth, spanLine[spanLine.length - 1][1]]);
    const optimalLine: Pt[] = spanLine.map(([x]) => [x, averageY] as Pt);
    spanPoints.push(spanLine);
    optimalPoints.push(optimalLine);
  }
  for (const line of spanPoints) {
    line.unshift([line[0][0], 0]);
    line.push([line[line.length - 1][0], imageHeight]);
  }
  for (const line of optimalPoints) {
    line.unshift([line[0][0], 0]);
    line.push([line[line.length - 1][0], imageHeight]);
  }
...
export function calculateDewarpTransformation(
  ...
  const source = spanPoints.flat();
  const destination = optimalPoints.flat();
